...
function ErrorPercentagePredictor() {
  ...
  const handlePredict2 = async (e) => {
    e.preventDefault();
    if (
      errorMessage ||
      !formValues["errorPercentageConcurrency"] ||
      !formValues["errorPercentageRateLimit"]
    ) {
    ...
    const response = await axios.post("/predict2", {
      concurrency: formValues["errorPercentageConcurrency"],
      rateLimit: formValues["errorPercentageRateLimit"],
    });
  ...
  useEffect(() => {
    const chart1 = new ApexCharts(chartRef.current, {
      ...
      series: predictions.map((prediction) => ({
        name: `Concurrency ${prediction.concurrency} | Rate Limit ${prediction.rateLimit}`,
      ...
      <div className="container-fluid bg-light py-4 rounded">
        <div className="container py-4">
          <div className="row mb-4">
            <p className="fs-2 fw-normal">Error Percentage Predictor</p>
          </div>

          <div className="row">
            <form
              className="row gy-2 gx-3 px-3 align-items-center"
              onSubmit={handlePredict2}
              onReset={clearForm}
            >
              <div className="col-auto row mt-2">
                <label
                  htmlFor="errorPercentageConcurrency"
                ...
                <input
                  ...
                  id="errorPercentageConcurrency"
                  name="errorPercentageConcurrency"
                  onChange={handleInputChange}
                  value={formValues["errorPercentageConcurrency"]}
                  required
              ...
              <div className="col-auto row mt-2">
                <label
                  htmlFor="errorPercentageRateLimit"
                  className="col-auto col-form-label"
                >
                  Rate Limit (Req/min)
                </label>
                <input
                  type="text"
                  className="col form-control"
                  id="errorPercentageRateLimit"
                  name="errorPercentageRateLimit"
                  onChange={handleInputChange}
                  value={formValues["errorPercentageRateLimit"]}
                  required
              ...
              <div className="col-auto mt-2">
                ...
                >
                  {/* {loading
                    ? "Predicting error percentage forecast..."
                    : "Predict"} */}
                  {loading ? (
                    <span
...
export default ErrorPercentagePredictor;
